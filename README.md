...
| `make test` | Запуск тестов в dev-окружении |
| `make coverage` | Покрытие тестами в терминале |
| `make coverage-html` | Покрытие тестами в HTML формате |
...
Для быстрой проверки работоспособности кода используйте:
```bash
> make test
```

### 2. Анализ покрытия кода (Test Coverage)
Чтобы проверить, какие строки кода и сценарии UseCase были задействованы в тестах, используйте встроенные инструменты отчетов:

* **Вывод в консоль:** Быстрый текстовый отчет со списком невыполненных строк (`Missing` lines):
```bash
> make coverage
```
* **Интерактивный HTML-отчет:** Генерация визуального дашборда:
```bash
> make coverage-html
```
*После выполнения команды откройте файл `htmlcov/index.html` в любом браузере. Вы сможете построчно увидеть,
какие ветки условий (`if/else`) в вашей бизнес-логике остались без тестов.*

### 3. Комплексная проверка перед коммитом (CI Pipeline)
