...
from src.core.users.domain.exceptions.exception_classes import UserAlreadyExistsError, MissingRequiredFieldError, \
    InvalidInputError, InvalidCredentialsError, EmailNotConfirmedError, InvalidEmailConfirmationCodeError, \
    ConfirmationCodeExpiredError, TokenCryptographyError, TokenStateError
...
def get_error_message(code: str, verbose_name: str) -> str:
    template = ERROR_MESSAGES.get(code, "Произошла ошибка")
...
def get_pydantic_error_message(code: str, verbose_name: str) -> str:
    template = PYDANTIC_ERROR_MESSAGES.get(code, "Произошла ошибка")
