...
class ChangeMachineryGeneralUseCase:
    def __init__(self, uow: IMachineryUnitOfWork):
        self.uow = uow

    async def execute(self, machinery_id: UUID, dto: ChangeMachineryGeneralRequest):
        async with self.uow as uow:
            machinery = await uow.machinery.get_machinery_by_id(machinery_id)
            machinery.change_general(dto.brand_id, dto.model, dto.color_id, dto.country_id)
