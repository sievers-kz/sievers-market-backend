...
class ChangeMachineryDescriptionUseCase:
    def __init__(self, uow: IMachineryUnitOfWork):
        self.uow = uow

    async def execute(self, machinery_id: UUID, dto: ChangeMachineryDescriptionRequest):
        async with self.uow as uow:
            machinery = await uow.machinery.get_machinery_by_id(machinery_id)
            machinery.change_description(dto.description)
