...
class Listing(BaseModel):
    ...
    owner_id: Mapped[UUID] = mapped_column(
        FK("vendors.id", ondelete="CASCADE"), nullable=False
    )
    category_id: Mapped[UUID] = mapped_column(
        FK("categories.id", ondelete="CASCADE"), nullable=False
    )
    subcategory_id: Mapped[UUID] = mapped_column(
        FK("subcategories.id", ondelete="RESTRICT"), nullable=False
    )
    ...
    city_id: Mapped[UUID] = mapped_column(
        FK("cities.id", ondelete="SET NULL"), nullable=True
    )
