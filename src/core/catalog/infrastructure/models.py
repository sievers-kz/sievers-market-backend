...
class Rubric(BaseModel):
    ...
    categories: Mapped[list["Category"]] = relationship(lazy="selectin", cascade="all, delete-orphan")
...
class SubcategoryAttribute(BaseModel):
    ...
    attribute_id: Mapped[UUID] = mapped_column(ForeignKey("attribute_definitions.id", ondelete="CASCADE"))
    group_id: Mapped[UUID] = mapped_column(ForeignKey("attribute_groups.id", ondelete="RESTRICT"))
    unit_id: Mapped[UUID | None] = mapped_column(ForeignKey("unit_of_measure.id", ondelete="SET NULL"), nullable=True)
