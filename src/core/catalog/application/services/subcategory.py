...
class SubcategoryService:
    ...
    async def create(self, dto: CreateSubcategoryRequest) -> None:
        async with self.uow as uow:
            ...
            subcategory = Subcategory.create(
                category_id=category.id, name=dto.name, attributes=attributes
            )
    ...
    async def change_category(
        self, subcategory_id: UUID, dto: ChangeSubcategoryParentRequest
    ):
        async with self.uow as uow:
            category = await uow.category.get_by_id(dto.category_id)
            if not category:
        ...
        logger.info(
            "Subcategory parent changed | subcategory_id={} parent_id={}",
            subcategory.id,
            category.id,
        )

    async def change_name(
        self, subcategory_id: UUID, dto: ChangeSubcategoryNameRequest
    ) -> None:
        async with self.uow as uow:
            subcategory = await uow.subcategory.get_by_id(subcategory_id)
            if not subcategory:
        ...
        logger.info(
            "Subcategory name changed | subcategory_id={} name={}",
            subcategory.id,
            subcategory.name,
        )

    async def replace_attributes(
        self, subcategory_id: UUID, dto: ReplaceSubcategoryAttributeRequest
    ):
        async with self.uow as uow:
            subcategory = await uow.subcategory.get_by_id(subcategory_id)
            if not subcategory:
            ...
            domain_attributes = [
                Attribute.from_dict(attr.model_dump()) for attr in dto.attributes
            ]
        ...
        logger.info(
            "Subcategory attributes changed | subcategory_id={}", subcategory.id
        )
    ...
    async def validate_attributes(
        self, subcategory_id: UUID, attributes: dict[str, Any]
    ):
        async with self.uow as uow:
            subcategory = await uow.subcategory.get_by_id(subcategory_id)
            if not subcategory:
