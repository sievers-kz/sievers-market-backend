...
class TestRefreshTokenUseCase:
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_fails_with_ghost_token(
        self,
        refresh_token_usecase,
        token_service
    ):
    ...
    @pytest.mark.asyncio
    @pytest.mark.integration
    async def test_fails_with_wrong_token_type(self, refresh_token_usecase, token_service):
        fake_user_id = uuid.uuid4()
    ...
    async def test_fails_on_reused_revoked_token(
        ...
        first_refresh_token_dto = RefreshData(refresh_token=login_response.refresh_token)
